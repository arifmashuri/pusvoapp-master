...
public class TopupGameActivity extends AppCompatActivity {
    ...
    @OnClick(R.id.buttonSubmit) public void ubah() {
        ...
        if (editTexthp.getText().toString().length() > 7 && editTextMeter.getText().toString().length() > 7 && idproduk!=null) {
            ...
            String user = editTextMeter.getText().toString();
            ...
            Call<Data> call = api.simpantransaksi( "Bearer "+sharedPrefManager.getSPBearer(),hp,user, idproduk, metodekirim,Enkripsi.vapps(context), randomUUIDString, SHA1, api_key, api_secret);
...
//                Toast.makeText(getApplicationContext(),nama,Toast.LENGTH_SHORT).show();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Top Up Game Online");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setHomeAsUpIndicator(R.drawable.kembali);
        jenismeter.setText(R.string.id_game);
        ...
        sk.setText(R.string.sktopupgame);
        ...
        Retrofit retrofit = new Retrofit.Builder()
                ...
                .build();
        API api = retrofit.create(API.class);
        Call<Data> call = api.dataproduk(13);
        call.enqueue(new Callback<Data>() {
            @Override
            public void onResponse(@NonNull Call<Data> call, @NonNull retrofit2.Response<Data> response) {
    ...
    public void model1(String hp, String meter) {
        ...
        if (hp.length() > 4 || meter.length() > 4) {
            ...
            boolean meterbenar = Enkripsi.validator(6,16,"nomor",meter);
